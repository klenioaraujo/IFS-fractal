...
if __name__ == "__main__":
    main()
"""
Fractal Dimension Analysis with Spectral Methods
===============================================

Theoretical Foundation:
- Fractal dimension D: Measure of complexity scaling as N(ε) ~ ε^(-D)
- Box-counting dimension: D = lim_{ε→0} log(N(ε)) / log(1/ε)
- Spectral dimension: Related to random walk properties on fractals
- IFS (Iterated Function Systems): Contractive mappings generating fractals

Mathematical Derivations:
- Hausdorff dimension for self-similar sets
- Spectral dimension from power spectrum analysis
- Box-counting via grid discretization

Empirical Validation:
- Tested on known fractals (Sierpinski triangle: D=1.585)
- Statistical significance of dimension estimates
- Convergence analysis for different scales

References:
- Falconer, K. (2003). Fractal Geometry: Mathematical Foundations and Applications
- Havlin, S., & Ben-Avraham, D. (2002). Diffusion in disordered media
"""
...
# =============================================================================
# 1. PARAMETRIC IFS FRACTAL GENERATOR
# =============================================================================
class FractalGenerator:
    def __init__(self, dim=2):
        self.dim = dim
...
# =============================================================================
# 2. LASER PULSE SIMULATOR
# =============================================================================
class LaserPulseSimulator:
    """
    Physically motivated laser pulse simulator for fractal interaction studies.

    Theoretical Foundation:
    - Gaussian beam propagation: E(r,z) ∝ exp(-r²/w²) exp(ikz - z²/z_R²)
    - Chirped pulses: E(t) ∝ exp(-t²/τ²) exp(iφ(t)) with φ(t) = ω₀t + (1/2)βt²
    - Fractal scattering: Differential cross-section σ(θ) ~ k^{-(D+1)} sin²(θ/2)

    Mathematical Model:
    - Pulse envelope: Gaussian in time and space
    - Phase: Linear chirp + quadratic spatial phase
    - Interaction: Distance-dependent scattering amplitude
    """

    def __init__(self, I0=1.0, omega=2*np.pi, alpha=0.1, k=2*np.pi, beta=0.05,
                 pulse_duration=1e-12, beam_waist=10e-6):
        """
        ...
        Args:
            ...
            alpha: Spatial modulation coefficient
            k: Wave number (rad/m)
            beta: Chirp parameter
            pulse_duration: Temporal pulse width (s)
        ...
        self.w0 = beam_waist       # Beam waist

    def pulse(self, lambda_val, t):
        """
        Calculate the laser pulse electric field at position lambda and time t.

        Uses Gaussian envelope with linear chirp and quadratic spatial phase.
        """
        # Gaussian temporal envelope
        temporal_envelope = np.exp(-t**2 / (2 * self.tau**2))

        # Gaussian spatial envelope (beam profile)
        spatial_envelope = np.exp(-lambda_val**2 / (2 * self.w0**2))

        # Phase: carrier + chirp + spatial phase
        phase = self.omega * t + self.beta * t**2 - self.k * lambda_val + self.alpha * lambda_val**2

        return self.I0 * temporal_envelope * spatial_envelope * np.exp(1j * phase)

    def interact_with_fractal(self, fractal_points, scan_direction='x', t_range=(0, 1), n_t=100):
        """
        Simulate physically motivated pulse interaction with fractal structure.

        Uses distance-dependent scattering cross-section based on fractal dimension.
        """
        # Determine fractal bounds
        min_val, max_val = np.min(fractal_points, axis=0), np.max(fractal_points, axis=0)
        extent = max_val - min_val

        # Scan parameters
        t_values = np.linspace(t_range[0], t_range[1], n_t)
        lambda_values = np.linspace(min_val[0] - 0.2*extent[0], max_val[0] + 0.2*extent[0], n_t)

        # Matrix to store response
        response = np.zeros((n_t, n_t), dtype=complex)

        # Calculate fractal dimension for scattering model
        fractal_gen = FractalGenerator(dim=fractal_points.shape[1])
        # Note: Would need to set transforms, but for simulation we'll use estimated D
        D_estimated = 1.7  # Approximate for demonstration
        ...
        for i, t in enumerate(t_values):
            for j, l in enumerate(lambda_values):
                ...
                # Calculate distance to nearest fractal point
                if scan_direction == 'x':
                    distances = np.abs(fractal_points[:, 0] - l)
                else:
                    distances = np.sqrt(np.sum((fractal_points - np.array([l, 0]))**2, axis=1))

                min_dist = np.min(distances)

                # Physically motivated scattering cross-section
                # σ ∝ 1/r^{D-1} for fractal scattering (approximate)
                if min_dist > 1e-10:
                    scattering_cross_section = min_dist ** (1 - D_estimated)
                else:
                    scattering_cross_section = 1.0

                # Attenuation due to beam propagation
                propagation_loss = np.exp(-0.1 * extent[0] / self.w0)

                # Total interaction amplitude
                interaction = scattering_cross_section * propagation_loss
                response[i, j] = pulse_val * interaction
